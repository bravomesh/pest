...
const MapComponent = forwardRef(({ layersVisibility, filteredLayers, onToggleLayer, onMapInitialized, selectedCountry }, ref) => {
  ...
  const colorRamps = useMemo(() => ({
    ...
    'Desert Locust Risk': {
      ...
      discreteClasses: [
        { value: 'High Risk', color: '#FFA500', range: '>6' },
        { value: 'Moderate Risk', color: '#FFDBBB', range: '3-6' },
        { value: 'Low Risk', color: '#FFFFFF', range: '<3' }
      ],
      unit: ''
    },
    'Habitat Suitability': {
  ...
  const overlayLayers = useMemo(() => ({
    ...
    "Habitat Suitability": L.tileLayer.wms('http://localhost:8080/geoserver/igad/wms', {
      layers: 'igad:rkslevel_view',
      format: 'image/png',
      transparent: true,
      styles: 'maxentoutputstyle',
    ...
    try {
      ...
      const popupContent = `
        <div>
          ...
          <button class="analytics-button" onclick="window.goToAnalytics()">ANALYZE</button>
        </div>
      `;
  ...
  const updateTreeControl = useCallback(() => {
    ...
    const createLegend = (colorRamp) => {
      const { colors, label, unit, discreteClasses } = colorRamp;
      ...
      const getValueRanges = (layerType) => {
        const ranges = {
          ...
          'Habitat Suitability': { high: '>6', moderate: '>3 <6', low: '<3' },
        };
        return ranges[layerType] || { high: '>6', moderate: '>3 <6', low: '<3>' };
      };
    
      // Check if the layer has discrete classes (like Desert Locust Risk)
      if (discreteClasses && label === 'Desert Locust Risk') {
        return `
          <div class="legend-item">
            <div class="legend-title">${label}</div>
            <div class="legend-labels">
              ${discreteClasses.map(cls => `
                <div class="legend-label">
                  <span class="legend-color" style="background-color: ${cls.color}; width: 12px; height: 12px; display: inline-block; margin-right: 5px;"></span>
                  <span class="legend-text">
                    <strong>${cls.value}</strong> <!-- Display risk class label -->
                    <span class="legend-unit">${unit}</span>
                    <br/>
                    <small>${cls.range}</small> <!-- Display range -->
                  </span>
                </div>
              `).join('')}
            </div>
      ...
      // For other layers, use the continuous color gradient legend
    ...
    const breedTree = {
      ...
      children: layersVisibility["Breeding Sites"]
        ? [
            {
              label: `Breeding Sites <div style="background-color: yellow; width: 15px; height: 1px; border-radius: 50%; display: inline-block;"></div>`,
    ...
    const outbreakTree = {
      ...
      children: layersVisibility["Habitat Suitability"]
        ? [
            {
              label: 'Habitat Suitability',
              layer: overlayLayersRef.current["Habitat Suitability"],
              children: [{ 
                label: createLegend(colorRamps['Habitat Suitability']),
                layer: null,
              }],
            },
          ]
    ...
    const pestsTree = {
      ...
      children: [
        ...
        layersVisibility["Hoppers"] && {
          label: `Hoppers <img src="hopper.png" style="width: 35px; height: 35px; display: inline-block; vertical-align: middle;">`,
    ...
    treeControlRef.current = L.control.layers.tree(baseTree, [
      riskTree,
      { label: '<hr style="border: 1px solid #ccc; margin: 1px 0;">' },
      breedTree,
      { label: '<hr style="border: 1px solid #ccc; margin: 1px 0;">' },
      outbreakTree,
      { label: '<hr style="border: 1px solid #ccc; margin: 1px 0;">' },
